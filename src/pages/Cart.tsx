              ...
              >
                ...
                {data?.stock && 
                  <div className="flex justify-center items-center gap-1 sm:gap-5 w-1/6">
                    <svg style={quantity>=2?{ cursor: 'pointer' }:{cursor: 'pointer', visibility:'hidden'}} onClick={() => { if(data?.id && quantity>=2) dispatch(addToCart({ id: data?.id, quantity: -1 })); }} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="#3b82f6" d="M18 16H6c-1.654 0-3-1.346-3-3s1.346-3 3-3h12c1.654 0 3 1.346 3 3s-1.346 3-3 3M6 12c-.551 0-1 .449-1 1s.449 1 1 1h12c.551 0 1-.449 1-1s-.449-1-1-1z" /></svg>
                    {quantity}
                    <svg style={quantity<=data?.stock?{ cursor: 'pointer' }:{cursor: 'pointer', visibility:'hidden'}} onClick={() => { if (data?.id && quantity <= data.stock) dispatch(addToCart({ id: data?.id, quantity: +1 })); }} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><g fill="none" fillRule="evenodd"><path d="M24 0v24H0V0zM12.594 23.258l-.012.002l-.071.035l-.02.004l-.014-.004l-.071-.036q-.016-.004-.024.006l-.004.01l-.017.428l.005.02l.01.013l.104.074l.015.004l.012-.004l.104-.074l.012-.016l.004-.017l-.017-.427q-.004-.016-.016-.018m.264-.113l-.014.002l-.184.093l-.01.01l-.003.011l.018.43l.005.012l.008.008l.201.092q.019.005.029-.008l.004-.014l-.034-.614q-.005-.018-.02-.022m-.715.002a.02.02 0 0 0-.027.006l-.006.014l-.034.614q.001.018.017.024l.015-.002l.201-.093l.01-.008l.003-.011l.018-.43l-.003-.012l-.01-.01z" /><path fill="#3b82f6" d="M9 5a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v4h4a2 2 0 0 1 2 2v2a2 2 0 0 1-2 2h-4v4a2 2 0 0 1-2 2h-2a2 2 0 0 1-2-2v-4H5a2 2 0 0 1-2-2v-2a2 2 0 0 1 2-2h4zm4 0h-2v5a1 1 0 0 1-1 1H5v2h5a1 1 0 0 1 1 1v5h2v-5a1 1 0 0 1 1-1h5v-2h-5a1 1 0 0 1-1-1z" /></g></svg>
                  </div>
                }
                    {data?.price && (
                ...
                <div className="flex justify-center sm:gap-5 w-1/6">
                    ...
                    <Tooltip title="Delete Product" className="cursor-pointer" onClick={()=>{handleOpenDeleteModal(id)}}>
