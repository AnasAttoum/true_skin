...
export default function Cart() {
  ...
  const handleDelete = () => {
    if (idRef.current) dispatch(deleteFromCart(idRef.current));
    handleCloseDeleteModal();
  };

  return (
    <>
      <HelmetDetails title="Cart" href="/cart" description="True Skin cart" />
      ...
      {!cart.length ? (
        ...
        <div style={{ minHeight: "calc(100vh - 175px)" }}>
                  ...
                  {data?.stock && (
                    <>
                      <div className="flex justify-center items-center gap-1 sm:gap-5 w-1/6">
                        <svg
                          ...
                          onClick={() => {
                            if (data?.id && quantity >= 2)
                              dispatch(
                                addToCart({ id: data?.id, quantity: -1 })
                              );
                          }}
                        ...
                        <span
                          style={
                            quantity > data.stock ? { color: "#f50a10" } : {}
                          }
                        >
                          {quantity} <br />
                          {quantity > data.stock ? (
                            <span className="text-xs">
                              Not available <br /> in stock
                            </span>
                          ) : (
                            ""
                          )}
          ...
          {cart.every((el) => {
            ...
            return el.quantity <= (data?.stock || 1);
          }) ? (
            <div className="flex flex-col gap-1 items-end my-5 p-5">
              <div className="text-[--primary] text-2xl font-extrabold w-full sm:w-1/4">Cart Totals</div>
              <div className="flex gap-10 w-full sm:w-1/4">
                <div className="text-gray-800">Products:</div>
                <div className="font-bold">{cart.length}</div>
              </div>
              <div className="flex gap-14 w-full sm:w-1/4">
                <div className="text-gray-800">Items:</div>
                <div className="font-bold">{cart.reduce((acc,el)=>{return acc+el.quantity},0)}</div>
              </div>
              <div className="flex gap-14 w-full sm:w-1/4">
                <div className="text-gray-800">Total:</div>
                <div className="font-bold">{cart.reduce((acc,el)=>{
                  const data = products.find((product) => {
                    return product.id === el.id;
                  });
              ...
              <div className="flex w-full sm:w-1/4 my-5">
                <Link to={'/checkout'} className="btn">Continue Payment</Link>
              </div>

            </div>
          ) : (
            <div className="flex flex-col gap-1 items-end my-5">
              <div className="text-[--primary] text-2xl font-extrabold w-full sm:w-1/3">Not all items available in stock yet</div>
            </div>
          )}
        </div>
      )}
